...
static int esCadenaDeNumeros(char cadena[], int limite);
//-------------------------------------------------------------------------------------------------
...
int ObtenerCadenaDeNumeros(char texto[], char mensaje[], char mensajeError[], int reintentos)
{
	...
	if(texto != NULL && mensaje != NULL && mensajeError != NULL)
	{
		do{
			printf("%s", mensaje);
			fflush(stdin);
			if(getString(bufferString,sizeof(bufferString))==0 && esCadenaDeNumeros(bufferString,sizeof(bufferString)))
			{
				strcpy(texto, bufferString);
				retorno=1;
				break;
	...
	for(i=0; i< limite && cadena[i] != '\0'; i++)
		{
			...
			if(cadena[i] > '9' || cadena[i] < '0' || (i==0 && cadena[i] == 32))
			{
				retorno= 0;
				break;
			}
